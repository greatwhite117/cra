...
def start(crl_keyword,crl_sdate,crl_edate,crl_cnt,list_path,logger) :
    ...
    if newscnt == 0 : # 기간내 전체 수집
       newscnt = int(get_newsCnt(crl_keyword,sdate1,crl_edate,sdate2))
    ...
    while  i <= loop_count : 
        ...
        time.sleep(3)
        counter = call_start(crl_keyword,sortOption,sdate1,sdate2,crl_edate,startPage,pageNum,counter,logger,wf)
        if counter == 0 :
            break
        i = i + 1
